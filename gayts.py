...
def build_loss(neural_net, optimizing_img, target_representations, content_feature_maps_index, style_feature_maps_indices, config):
    ...
    style_loss = 0.0
    current_style_representation = [util.gram_matrix(x) for cnt, x in enumerate(current_set_of_feature_maps) if cnt in style_feature_maps_indices]
    for gram_gt, gram_hat in zip(target_style_representation, current_style_representation):
        style_loss += torch.nn.MSELoss(reduction='sum')(gram_gt[0], gram_hat[0])
    style_loss /= len(target_style_representation)

    #! 计算图像的水平，竖直方向的一阶差分的总和
    tv_loss = util.total_variation(optimizing_img)
...
def neural_style_transfer(config):
    ...
    content_img = util.prepare_img(content_img_path, config['height'], device)
    style_img = util.prepare_img(style_img_path, config['height'], device)
    ...
    if config['init_method'] == 'random':
        ...
        style_img_resized = util.prepare_img(style_img_path, np.asarray(content_img.shape[2:]), device)
        init_img = style_img_resized

    # 优化的目标是图像! (that's why requires_grad=True)
    optimizing_img = Variable(init_img, requires_grad=True)

    neural_net, content_feature_maps_index_name, style_feature_maps_indices_names = util.prepare_model(config['model'], device)
    print(f'Using {config["model"]} in the optimization procedure.')

    #! 内容图像和风格图像分别喂给VGG，并只取需要的特定层的特征图
    content_img_set_of_feature_maps = neural_net(content_img)
    style_img_set_of_feature_maps = neural_net(style_img)

    target_content_representation = content_img_set_of_feature_maps[content_feature_maps_index_name[0]].squeeze(axis=0)
    target_style_representation = [util.gram_matrix(x) for cnt, x in enumerate(style_img_set_of_feature_maps) if cnt in style_feature_maps_indices_names[0]]
    ...
    if config['optimizer'] == 'adam':
        ...
        for cnt in range(num_of_iterations[config['optimizer']]):
            ...
            with torch.no_grad():
                ...
                util.save_and_maybe_display(optimizing_img, dump_path, config, cnt, num_of_iterations[config['optimizer']], should_display=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_video", action='store_true', help="save optimization process")
    ...
    util.make_gif(results_path, args.content_img_name + args.style_img_name, duration=0.03)
