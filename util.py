...
def generate_out_img_name(config):
    prefix = os.path.basename(config['content_img_name']).split(
        '.')[0] + '_' + os.path.basename(config['style_img_name']).split('.')[0]
...
def save_and_maybe_display(optimizing_img, dump_path, config, img_id, num_of_iterations, should_display=False):
    ...
    if img_id == num_of_iterations-1 or (saving_freq > 0 and img_id % saving_freq == 0):
        ...
        out_img_name = str(img_id).zfill(
            img_format[0]) + img_format[1] if saving_freq != -1 else generate_out_img_name(config)
...
def prepare_model(model, device):
    ...
    content_fms_index_name = (
        content_feature_maps_index, layer_names[content_feature_maps_index])
...
def make_gif(img_folder: str, gif_name: str, duration=0.1):
    start_time = time.perf_counter()
    if not gif_name.endswith('.gif'):
        gif_name = gif_name + '.gif'
    path = Path(img_folder)
    img_list = sorted(list(path.iterdir()), key=lambda x: len(str(x)))
    if len(img_list) == 0:
        print(img_folder, "is empty!!!")
        return
    gif_imgs = [iio.imread(img) for img in img_list]
    git_path = Path.joinpath(Path('.'), gif_name)
    iio.mimsave(git_path, gif_imgs, format='GIF', duration=duration)
    optimize(git_path)
    end_time = time.perf_counter()
    total_time = end_time - start_time
    print(f'Finished！ Took {total_time:.4f} seconds')
